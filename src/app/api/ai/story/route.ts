...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.AI_MOCK_MODE === "true") {
      const text = mockStory(body.contentMode, body.topic, target);
      return ok({ text, characterCount: countGraphemes(text) });
    }

    let text = await textRequest([
      { role: "system", content: STORY_SYSTEM },
      { role: "user", content: `주제: ${body.topic}\n모드: ${body.contentMode}\n목표: ${target}자\n최근 비유: ${(body.recentMetaphors ?? []).join(", ")}\n원고만 작성.` },
    ], { timeoutMs: 60000 });

    for (let attempt = 0; attempt < 3 && !withinTarget(text, target); attempt++) {
      const current = countGraphemes(text);
      text = await textRequest([
        { role: "system", content: REVISION_SYSTEM },
        { role: "user", content: `현재 ${current}자, 목표 ${target}자(허용 ${target - 30}~${target + 30}). 기존 의미/인물/전개/결말을 유지하고 길이만 조정.\n\n기존 원문:\n${text}` },
      ], { timeoutMs: 60000 });
    }
    return ok({ text, characterCount: countGraphemes(text) });
  } catch (error) {
    console.error("story", error);
    return fail("STORY_FAILED", friendlyAiError(error, "이야기를 만들지 못했어요. 다시 시도해주세요."));
  }
}
